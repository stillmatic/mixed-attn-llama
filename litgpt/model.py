...
class Block(nn.Module):
    ...
    def _get_attention_layer(self, config: Config, layer_idx: int) -> nn.Module:
        if not config.interleave_attn:
            return CausalSelfAttention(config, is_local=False)
        # print("Using interleaved attention")
        if (layer_idx + 1) % config.global_attn_interval == 0:
            return CausalSelfAttention(config, is_local=False)
        else:
            return CausalSelfAttention(config, is_local=True)
...
class CausalSelfAttention(nn.Module):
    def __init__(self, config: Config, is_local: bool = False) -> None:
        super().__init__()
        ...
        # if we are operating as a local attn layer
        if is_local:
    ...
    def forward(
        ...
        if self.is_local:
            y = self.local_attention(q, k, v, mask=mask)
        else:
    ...
    def _get_local_mask(self, shape: torch.Size, block_size: int) -> torch.Tensor:
        ...
        # for static kv-cache, we need to pad the mask with additional falses so the
        # result goes from (C, C) to (C, block_size)
        mask = torch.cat(
